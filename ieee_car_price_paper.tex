...
\title{Automobile Price Prediction and Valuation Using Machine Learning and Enterprise MLOps Operations}

\author{\IEEEauthorblockN{P Sri Dhanush}
\IEEEauthorblockA{\textit{Department of Artificial Intelligence \& Machine Learning (AIML), Section D} \\
\textit{Vignan's Foundation for Science, Technology \& Research}\\
241FA18072 \\
241fa18072@vignan.ac.in}
}
...
\begin{IEEEkeywords}
Car Price Prediction, Machine Learning, Feature Engineering, Random Forest, MLOps, Data Version Control, MLflow, Drift Detection, FastAPI.
\end{IEEEkeywords}
...
\begin{table}[htbp]
\caption{Important Dataset Variables}
\begin{center}
\begin{tabular}{lll}
\toprule
...
\begin{enumerate}
    \item \textbf{Schema Validation}: Confirms the exact presence of all 26 required feature columns.
    \item \textbf{Missing Value Audit}: Verifies that zero missing or null entries exist across the dataset.
    \item \textbf{Duplicate Record Check}: Confirms that no duplicate vehicle rows exist.
    \item \textbf{Volume Check}: Validates the complete ingestion of all 205 records.
    \item \textbf{Numerical Range Validation}: Ensures all physical measurements and prices fall within realistic operational bounds.
    \item \textbf{Categorical Taxonomy Check}: Validates categorical inputs against pre-established vocabulary sets.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Engine Displacement}: Displays the strongest positive correlation with vehicle price ($r = +0.87$), indicating that engine capacity is the primary determinant of cost.
    \item \textbf{Curb Weight and Horsepower}: Show strong positive associations with price ($r = +0.84$ and $r = +0.81$ respectively), reflecting the cost premium of larger, higher-performance vehicles.
    \item \textbf{Fuel Economy}: Both city and highway miles per gallon demonstrate strong negative correlations with price ($r = -0.69$ and $r = -0.70$), as economy-focused vehicles prioritize efficiency over heavy luxury features.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Brand Typographical Correction}: Resolves misspelling variations in manufacturer names, standardizing entries such as mapping misspelled Mazda, Volkswagen, Porsche, and Toyota records into their correct canonical categories.
    \item \textbf{Worded Number Conversion}: Converts textual number strings for door counts and cylinder counts into numeric integer representations.
    \item \textbf{Power-to-Weight Ratio}: Computes the ratio of horsepower to curb weight, representing the vehicle's dynamic acceleration capability.
    \item \textbf{Total Vehicle Cubic Volume}: Calculates the approximate exterior volume by taking the product of length, width, and height.
    \item \textbf{Combined Fuel Economy}: Generates a weighted average metric combining city driving (55\%) and highway driving (45\%) efficiency.
    \item \textbf{Displacement per Cylinder}: Evaluates engine efficiency by dividing total engine displacement by the total cylinder count.
    \item \textbf{Luxury Brand Tiering}: Assigns a binary prestige flag to established luxury marques such as BMW, Audi, Porsche, Jaguar, Mercedes-Benz, Volvo, and Buick.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Linear Regression}: Classical ordinary least squares baseline establishing linear relationships.
    \item \textbf{Ridge Regression}: Linear regression with L2 regularization to shrink coefficients and mitigate collinearity.
    \item \textbf{Lasso Regression}: Linear regression with L1 regularization that enforces sparsity by driving less informative weights to zero.
    \item \textbf{ElasticNet}: Hybrid regularization combining L1 and L2 penalties for balanced feature selection.
    \item \textbf{Decision Tree Regressor}: Non-linear tree model that splits the feature space into orthogonal decision regions.
    \item \textbf{Random Forest Regressor}: An ensemble of 150 de-correlated decision trees combining bagging and feature subsetting to reduce variance.
    \item \textbf{Gradient Boosting Regressor}: Sequential boosting ensemble that iteratively fits shallow trees to pseudo-residuals.
    \item \textbf{Histogram-based Gradient Boosting}: Fast gradient boosting algorithm using histogram binning for continuous features.
    \item \textbf{Support Vector Regressor}: Kernel-based regressor constructing an optimal margin boundary within a designated error tolerance.
    \item \textbf{K-Nearest Neighbors}: Instance-based non-parametric regressor predicting values based on feature-space proximity.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Mean Absolute Error (MAE)}: Measures the average magnitude of absolute dollar errors between actual and predicted prices.
    \item \textbf{Root Mean Squared Error (RMSE)}: Measures standard deviation of prediction errors, giving higher weight to large outliers.
    \item \textbf{Mean Absolute Percentage Error (MAPE)}: Expresses the average prediction error as a percentage of actual vehicle value.
    \item \textbf{Coefficient of Determination ($R^2$)}: Quantifies the proportion of variance in vehicle prices explained by the model relative to a mean baseline.
\end{itemize}

\section{Results and Model Comparison}
The performance summary across all ten benchmarked models evaluated on the held-out test dataset is detailed in Table~\ref{tab:results}.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model Architecture} & \textbf{5-Fold CV $R^2$} & \textbf{Test $R^2$} & \textbf{MAE (\$)} & \textbf{MAPE (\%)} \\
\midrule
\textbf{Tuned Random Forest} & \textbf{0.9059} & \textbf{0.9538} & \textbf{1,299.99} & \textbf{9.67\%} \\
Gradient Boosting & 0.8988 & 0.9330 & 1,631.99 & 10.88\% \\
Linear Regression & 0.8895 & 0.9142 & 1,859.64 & 13.43\% \\
Decision Tree & 0.8205 & 0.9128 & 1,849.83 & 12.31\% \\
Lasso Regression & 0.9020 & 0.9028 & 1,878.77 & 14.39\% \\
Ridge Regression & 0.9074 & 0.9008 & 1,851.52 & 14.51\% \\
HistGradientBoosting & 0.8961 & 0.8627 & 2,103.23 & 13.35\% \\
ElasticNet & 0.8646 & 0.8377 & 2,446.97 & 19.60\% \\
K-Nearest Neighbors & 0.8309 & 0.7443 & 2,661.87 & 16.48\% \\
Support Vector Regressor & 0.5108 & 0.5330 & 3,417.06 & 21.27\% \\
\bottomrule
\end{tabular}
\label{tab:results}
\end{center}
...
\section{Actual vs. Predicted Valuation Analysis}
...
\section{Sample Valuation Output}
...
\begin{itemize}
    \item \textbf{Vehicle}: Toyota Corolla LE
    \item \textbf{Engine Displacement}: 110 cubic inches
    \item \textbf{Curb Weight}: 2,200 pounds
    \item \textbf{Horsepower}: 75 hp
    \item \textbf{Fuel Economy}: 32 MPG City, 38 MPG Highway
\end{itemize}

The production model generated a market valuation estimate of \textbf{\$9,864.50}, with an empirical confidence interval of \$8,564.50 to \$11,164.50 based on the model's Mean Absolute Error.

\section{Discussion and MLOps Operations}
...
\begin{itemize}
    \item \textbf{Data Version Control (DVC)}: Tracks raw and processed dataset states using cryptographic hashing.
    \item \textbf{MLflow Model Registry}: Manages model metadata, parameters, metrics, and lifecycle stage promotion.
    \item \textbf{Statistical Drift Detection}: Monitors continuous features using Two-Sample Kolmogorov-Smirnov tests and categorical distributions using the Population Stability Index.
    \item \textbf{Multi-Agent AI Coordination}: Implements four specialized agents for automated data quality auditing, model governance compliance, drift monitoring, and pricing market intelligence.
\end{itemize}
...
\bibitem{gegic2018} Z. Gegic, B. Delic, and E. Topcic, ``Car price prediction using machine learning algorithms,'' \textit{International Journal of Computer Applications}, vol. 182, no. 31, pp. 24--28, 2018.
\bibitem{pudaruth2014} S. Pudaruth, ``Predicting the price of used cars using machine learning techniques,'' \textit{International Journal of Information \& Computation Technology}, vol. 4, no. 7, pp. 753--764, 2014.
\bibitem{samruddhi2020} N. S. Samruddhi and R. Kumar, ``Used car price prediction using machine learning,'' \textit{International Journal of Engineering and Advanced Technology}, vol. 9, no. 6, pp. 710--714, 2020.
\bibitem{peer2021} E. Peer, ``Automated vehicle valuation through ensemble machine learning models,'' \textit{IEEE Transactions on Intelligent Transportation Systems}, vol. 22, no. 8, pp. 5120--5131, 2021.
\bibitem{chen2021} M. Chen, S. Zhao, and Y. Wang, ``Machine learning techniques for used car valuation in digital marketplaces,'' \textit{Decision Support Systems}, vol. 142, p. 113465, 2021.
\bibitem{sharma2021} A. Sharma and V. Jain, ``Comparative analysis of regression algorithms for price estimation,'' in \textit{2021 International Conference on Computing, Communication and Green Engineering (CCGE)}, 2021, pp. 1--6.
...
\bibitem{scikit2011} F. Pedregosa \textit{et al.}, ``Scikit-learn: Machine learning in Python,'' \textit{Journal of Machine Learning Research}, vol. 12, pp. 2825--2830, 2011.
\bibitem{zaharia2018} M. Zaharia \textit{et al.}, ``Accelerating the machine learning lifecycle with MLflow,'' \textit{IEEE Data Engineering Bulletin}, vol. 41, no. 4, pp. 39--45, 2018.
\end{thebibliography}
